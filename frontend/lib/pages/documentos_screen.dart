...
import 'package:frontend/pages/visor_pdf_screen.dart';
...
class _DocumentosScreenState extends State<DocumentosScreen> {
  ...
  // --- OBTENCIÓN DE DATOS DEL PROVIDER ---
  String _getToken() => Provider.of<UserProvider>(context, listen: false).token!;
  String _getUsername() => Provider.of<UserProvider>(context, listen: false).user!.username;

  Future<void> _cargarDatos() async {
    setState(() => _isLoading = true);
    try {
      final token = _getToken();
      final communityProv = Provider.of<CommunityProvider>(context, listen: false);
  ...
  Future<void> _cargarDocumentosDeCarpeta() async {
    ...
    try {
      _documentos = await _docService.getDocumentos(_carpetaActual!.id, _getToken());
    } catch (e) {
  ...
  Future<void> _subirPDF() async {
    try {
      ...
      if (result != null) {
        if (_carpetaActual == null) {
          _mostrarMensaje("Error: No hay ninguna carpeta seleccionada", Colors.orange);
          return;
        }
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              setState(() => _isLoading = true);
              try {
                final comunidad = Provider.of<CommunityProvider>(context, listen: false).communityName!;
                // Actualizado con username
                await _docService.crearCarpeta(controller.text, comunidad, _getToken(), _getUsername());
                await _cargarDatos();
              } catch (e) {
  ...
  Future<void> _moverDocumento(Documento doc) async {
    final carpetasDestino = _carpetas.where((c) => c.id != _carpetaActual!.id).toList();
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: carpetasDestino.map((c) => ListTile(
            ...
            onTap: () async {
              ...
              try {
                // Actualizado con username
                await _docService.moverDocumento(doc.id, c.id, _getToken(), _getUsername());
              ...
              children: [
                        ...
                        child: ChoiceChip(
                          ...
                          onSelected: (selected) {
                            if (selected) {
                              ...
                              _cargarDocumentosDeCarpeta().then((_) => setState(() => _isLoading = false));
                            }
                          },
                        ),
                      );
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      PopupMenuButton<String>(
                        ...
                        onSelected: (value) async {
                          ...
                          if (value == 'borrar') {
                            try {
                              setState(() => _isLoading = true);
                              // Actualizado con username
                              await _docService.borrarCarpeta(_carpetaActual!.id, _getToken(), _getUsername());
                              _carpetaActual = null;
                              await _cargarDatos();
                            } catch (e) {
                        ...
                        itemBuilder: (context) => [
                          const PopupMenuItem(value: 'crear', child: Text("Crear Nueva Carpeta")),
                          if (_carpetaActual != null && !_carpetaActual!.esPrincipal)
                            const PopupMenuItem(value: 'borrar', child: Text("Borrar Carpeta Actual", style: TextStyle(color: Colors.red))),
                        ],
                      ),
                    ],
                  ),
                ...
                Expanded(
                  child: _documentos.isEmpty
                      ? const Center(child: Text("Carpeta vacía. No hay PDFs.", style: TextStyle(color: Colors.grey)))
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return Card(
                              ...
                              child: ListTile(
                                ...
                                subtitle: Text("Subido por: ${doc.creadorUsername}"),
                                onTap: () {
                                  Navigator.push(
                                    context,
                                    MaterialPageRoute(
                                ...
                                trailing: Row(
                                  ...
                                  children: [
                                    IconButton(
                                      icon: const Icon(Icons.drive_file_move_outline, color: Colors.teal),
                                      ...
                                      onPressed: () async {
                                        final confirmar = await showDialog<bool>(
                                          ...
                                          builder: (ctx) => AlertDialog(
                                            ...
                                            content: Text("¿Estás seguro de que quieres eliminar '${doc.titulo}'?"),
                                            actions: [
                                        ...
                                        if (confirmar == true) {
                                          try {
                                            setState(() => _isLoading = true);
                                            // Actualizado con username
                                            await _docService.borrarDocumento(doc.id, _getToken(), _getUsername());
                                            _mostrarMensaje("Archivo eliminado correctamente", Colors.green);
                                            await _cargarDatos();
                                          } catch (e) {
                                            _mostrarMensaje("Error al borrar: $e", Colors.red);
                                            setState(() => _isLoading = false);
                                          }
                                        }
                                      },
